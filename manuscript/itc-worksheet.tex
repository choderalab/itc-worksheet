...
\author[qb3]{Sarah E.~Boyce\fnref{gilead}}
...
\author[qb3]{John D.~Chodera\corref{cor1}}
%\fntext[label2]{label2}
\cortext[cor1]{Corresponding author.}
\ead{jchodera@berkeley.edu}
...
Here, we show how to reduce precision- and accuracy-limiting errors while obtaining good estimates while minimizing material and time consumed by an experiment.
...
If the ligand is already in solution (e.g.~in DMSO stock), it may not be possible to fully eliminate excipients, leading to potential heat effects due to buffer mismatch even if attempts are made to match compositions~\cite{tellinghuisen-boyce-chodera:2012:anal-biochem:blanks}.
...
%\section*{Titrate preparation}

%Bovine carbonic anhydrase (Sigma-Aldrich, cat no.~C2522, 80\% purity) was purchased as a lyophylized powder. 
%Following the assay conditions outlined in Myszka et al.~\cite{myszka:2003:j-biomol-tech:abrf-mirg02}, 2 L of PBS buffer was prepared for overnight dialysis of the protein sample. 
%The protein solution was prepared by dissolving $25\pm0.1$ mg in $4\pm0.1$ mL of PBS buffer. 
%The concentration of the resulting solution was determined prior- and post-dialysis spectrophotometrically on a Hewlett-Packard Agilent UV-Vis spectrophotometer model 8453, using the molar extinction coefficient $\epsilon_{280 \:\mathrm{nm}}$  = 50070 M$^{-1}$ cm$^{-1}$~\cite{myszka:2003:j-biomol-tech:abrf-mirg02}. 
%The sample was then diluted to [M]$_0$ $\sim$ 10 $\mu$M using the purity-corrected post-dialysis concentration. 
%Because the site parameter $n$ absorbs errors in [M]$_0$ during the fitting procedure [Tellinghuisen], high precision is not required for these steps; a less precise spectrophotometer (such as the popular NanoDrop ND-1000) would suffice.

%\section*{Titrant preparation}

The titrand solution, bovine CAII (Sigma-Aldrich, cat no.~C2522, $\sim$29 kDa{\color{red}[JDC: Check MW.]}, Lot No.~071M6261) in PBS buffer, was prepared following the assay conditions outlined by Myszka et al.~\cite{myszka:2003:j-biomol-tech:abrf-mirg02}.  
...
The recovered protein was spun for 30 min at 16300 RPM with no visible precipitate observed.
The dialysate was filtered again and used to prepare both titrant and tetrad to minimize buffer mismatch heats during the ITC experiment.
...
{\color{red}[JDC: Should we include the error in $\epsilon$ here?  Or perhaps we should drop the error bars since high precision isn't required?]}
...
By contrast, care must be taken to minimize inaccuracies in preparing titrant solutions, because the standard data analysis algorithms treat [X]$_s$ as exactly known.  
...
For our chosen [X]$_s$ $\sim$ 720, this requires only 0.3 mg of CBS, but given our analytical balance precision ($\pm$0.1 mg), this would yield 33\% uncertainty in [X]$_s$, and hence the final relative errors in $K$ and $\Delta H$ would be \emph{at least} this large.  
{\color{red}[JDC: Which model balance was used for this?]}
...
Note that gravimetric solution preparation---in which the mass of both compound and solvent is used to determine the final concentration---could have been used instead, eliminating the need to ensure all solutions are at the appropriate temperature for volumetric glassware or pipettes.
{\color{red}[JDC: Can we say something about volume variation with temperature and how much of a problem this is expected to be?]}
Automated systems for gravimetric solution preparation and concentration error determination are available, though more commonly used in industrial settings.
...
However, since the uncertainty in the absorbance measurement is 1\%, the uncertainty in the extinction coefficient $\epsilon$ (10\%) would dominate the concentration error, resulting in a spectrophotometrically-determined concentration that is uncertain by 10\%. 
Indeed, the concentration we measure in this manner---700(70) $\mu$M {\color{red}[JDC: I have to re-measure the absorbance of the latest titrant solution.]}---is consistent with that determined by mass and volume, but is an order of magnitude more uncertain; had we chosen to use this spectrophotometrically-determined concentration for [X]$_s$, our final uncertainties in $K$ and $\Delta H$ would be at least 10\%.
...
{\color{red}[JDC: Joel, is this statement not sufficient, or do you think we really need to include the $V_0$ calibration results?]}
...
This can have important consequences in trying to ascribe significance to differences in entropy-enthalpy compensation within a congeneric series, especially when differences in $\Delta G$ are small~\cite{freire:chem-biol-drug-des:2007:compensation,freire:drug-discovery-today:2008:entropy-enthalpy,freire:nat-rev-drug-discovery:2010:itc-lead-optimization}.
...
JDC is funded by a Distinguished Postdoctoral Fellowship from the California Institute for Quantitative Biosciences (QB3) at the University of California, Berkeley.
...
Figure~\ref{figure:spreadsheet} depicts the spreadsheet (available for for download from Supplementary Material in multiple formats, and online at \url{http://simtk.org/home/itc-worksheet}) with the details for the CBS-CAII titration experiment reported here filled in.
...
Importantly, during both preparation of the titrant and titrate, a ``typical error'' sets the upper bound for the error the experimenter should be able to achieve.
...
{\color{red}[JDC: Should we add step-by-step instructions for filling in and using the spreadsheet?]}


{\color{red}[JDC: To propagate the error in $n$, I also used the rule of adding in quadrature:
\begin{equation}
\left(\frac{s_n}{n}\right)^2 = \left(\frac{s_{n,\mathrm{LS}}}{n}\right)^2 + \left(\frac{s_\mathrm{[X]_s}}{[X]_s}\right)^2 .
\end{equation}
If this is inappropriate, I should change this---I haven't worked through this yet.]}
...
This spreadsheet and blank templates is available for download in multiple formats at \url{http://simtk.org/home/itc-worksheet}.
