...
\color{red}
Note that had we only included this ``throwaway'' injection but not included the ``down syringe'' command, the additional titrant shortfall introduced by the worm gear reversal would have led to potentially significant errors in computed concentrations for all injections during analysis.
\color{black}
...
\color{red}
The NanoDrop (and similar instruments) utilize small sample volumes (3 $\mu$L was used here) and a 1 mm dynamic path length such that even high protein concentrations can be quickly read without the need to perform intermediate dilutions to ensure the absorbance falls in the linear range of Beer's law on standard 10 mm path-length spectrometers.
The measured absorbance of 11.8$\pm$0.2 at effective 10 mm path length indicated a protein concentration of 235$\pm$4 $\mu$M (assuming the molar extension coefficient $\epsilon_\mathrm{280\:nm} = 50070$ M$^{-1}$ cm$^{-1}$~\cite{myszka:2003:j-biomol-tech:abrf-mirg02} is known precisely).
\color{black}
The sample was then diluted to [M]$_0$ $\approx$ 10 $\mu$M using the purity-corrected post-dialysis concentration. 
\color{red}
Note that, because the site parameter $n$ absorbs errors in [M]$_0$ and $V_0$ in standard least-squares data analysis~\cite{tellinghuisen:2012:anal-biochem:designing-itc-experiments}, high precision is not required for these steps unless the stoichiometry is unknown.
\color{black}
...
\color{red}
To do this, we added 10.0$\pm$0.1 mg [henceforth written 10.0(1) mg] CBS to 32.1(2) mL PBS dialysate and vortexed to ensure the compound was completely dissolved, yielding 32.2(2) mL of a 1 500(20) $\mu$M CBS stock solution.  
\color{black}
...
\color{red}
Using the Gilson P5000, we then added 4.309(12) mL CBS stock to 4.691(12) mL PBS to obtain a 719(9) $\mu$M CBS titrant (1.3\% RSE).  
\color{black}
...
The use of volumetric glassware in principle requires all solutions and glassware to be equilibrated to the glassware calibration temperature, but due to the low coefficient of thermal expansion of borosilicate glass, this expansion will only introduce an error of 0.0010\%/$^\circ$C~\cite{lembeck:nbsir-74-461:calibration-of-volumetric-glassware}---small enough to be negligible for our purposes.
Note that gravimetric solution preparation (GSP)---in which the mass of both compound and solvent is used to determine the final concentration---could have been used instead, but runs the risk of introducing more significant errors due to the larger coefficient of thermal expansion of liquids (dilute aqueous buffers have a coefficient of thermal expansion near that of water, $\sim$0.025\%/$^\circ$C); however, this still generally amounts to a negligible contribution to error for calorimetry even for changes of several degrees.
...
\color{red}
Here, the blank heats were small and uniform, of the same magnitude as water-into-water injections (see Supplementary Material).  
\color{black}
...
If $f$ can be written $f(x,y,z,\ldots) = x^i y^j z^k \cdots$, then Eq.~\ref{equation:propagation-of-error} assumes the simple form,
\begin{equation}
...
%Had we instead made $\sim$10\% error in [X]$_s$ as the participates in the ABRF-MIRG'02 study did, our uncertainties would have increased to 0.06 kcal/mol in $\Delta G$, 1.08 kcal/mol in $\Delta H^\circ$, 
...
The RSEs of our results are quite good---3\% in $K_a$ and 1\% in $\Delta H^\circ$---and in line with the predicted errors from our initial experimental modeling step.

{\bf Discussion.}
Notably, had we instead inadvertently made a 10\% error in the titrant concentration [X]$_s$ during its preparation through a precision-degrading step (such as massing out less than 10 mg or using low-precision liquid transfer devices such as serological pipettes or graduated cylinders), this could have easily led to RSEs in $K_a$ and $\Delta H^\circ$ of 10\% or greater. 
Although the absolute error in $\Delta G^\circ$ would remain small ($\sim$0.04 kcal/mol), the absolute error in $\Delta H^\circ$ would be large ($\sim$1.1 kcal/mol), making the error in $-T \Delta S^\circ$ comparable in magnitude but opposite in sign.  
...
If indeed concentrations errors were at fault, simply repeating the experiment with the same solutions and that simply repeating the experiment using the same titrant solution or ligand stock solution (from which the titrant is prepared) would not reveal this variation~\cite{tellinghuisen-chodera:2011:anal-biochem:systematic-itc-errors}.
\color{black}
