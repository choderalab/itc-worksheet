...
Our VP-ITC instrument has a syringe assembly that utilizes a worm gear which, after the recommended purge-refill process, will cause a titrant shortfall in the first injection unless a ``down syringe'' command is issued prior to loading the syringe into the sample cell~\cite{tellinghuisen:anal-biochem:2004:first-injection-anomaly}; we therefore executed a 10~$\mu$L ``down syringe'' command immediately after the purge-refill cycle.
\color{red}
Note that newer calorimeters such as the Malvern/GE/MicroCal iTC-200 that utilize a $\sim$200 $\mu$L cell volume have also been observed to suffer from this syringe backlash phenomena;
a recent analysis found a 1.3$\mu$L shortfall in titrant delivery contributing as much as 8\% to error in $K$ and $n$~\cite{tellinghuisen:2015:bba-general:optimizing-itc}.
Because the stoichiometric endpoint is reached after a much smaller total amount of titrant has been delivered for these low-volume instruments---the iTC-200 syringe has a total usable volume of only 35 $\mu$L---the importance of ensuring a ``down syringe'' command is issued prior to loading the syringe into the sample cell is critical for achieving high accuracy.
\color{black}
